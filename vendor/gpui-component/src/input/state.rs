...
use crate::input::{
    ...
    element::{LINE_NUMBER_RIGHT_MARGIN, RIGHT_MARGIN},
...
impl InputState {
    ...
    pub fn text_geometry(&self) -> Option<TextGeometry> {
        let layout = self.last_layout.as_ref()?;
        let bounds = self.last_bounds?;
        let gutter = if layout.line_number_width == px(0.) {
            px(0.)
        } else {
            layout.line_number_width - LINE_NUMBER_RIGHT_MARGIN
        };
        Some(TextGeometry {
            origin: bounds.origin + point(layout.line_number_width, px(0.)),
            line_height: layout.line_height,
            viewport: Bounds::from_corners(
                point(self.input_bounds.left() + gutter, self.input_bounds.top()),
                self.input_bounds.bottom_right(),
            ),
        })
    }
